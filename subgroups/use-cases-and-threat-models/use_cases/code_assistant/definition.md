...
## CycloneDX 2.0 — Use Cases (`useCase` objects)

The following JSON blocks encode each use case and sub use case using the `useCase` object definition from the [CycloneDX 2.0 use case schema](https://github.com/CycloneDX/specification/blob/2.0-dev-threatmodeling/schema/2.0/model/cyclonedx-usecase-2.0.schema.json).
Actor references (`actors` array) correspond to `bom-ref` values defined in the `actors` encoding block above.

### Use Case: Code Assistant (top-level)
...
### Sub Use Case 1a: Fully Local Agentic Execution
...
### Sub Use Case 1b: External SaaS Agentic Execution
