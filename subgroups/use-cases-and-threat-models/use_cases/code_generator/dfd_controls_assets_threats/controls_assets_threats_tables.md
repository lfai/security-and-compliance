...
| T05 | Exposure of local secret/config                                          | Accident/mistake accidentally commits API tokens to local repository, exposing secrets                            | Information Disclosure | platform | OWASP A02, A06                               | CWE-200, CWE-312 | Workstation: Local codebase clone | Intellectual Property |
...
| T33 | Model generates insecure code from dataset                               | The model generates insecure code patterns (e.g., weak crypto, vulnerable constructs) based on its training dataset, potentially introducing vulnerabilities into user systems | Integrity / Information Disclosure | AI | OWASP GenAI LLM Top 10: Insecure Outputs; MITRE ATLAS: Adversarial Output | CWE-310, CWE-327, CWE-330 | Cloud Service: Assistant LLM | Harmful code generation |
